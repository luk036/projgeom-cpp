...
TEST_CASE("Property-based test: cross product anti-commutative") {
    rc::check("cross(a, b) == -cross(b, a)", []() {
        ...
        auto cross_ab = std::array<int64_t, 3>{a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2],
                                               a[0] * b[1] - a[1] * b[0]};
        auto cross_ba = std::array<int64_t, 3>{b[1] * a[2] - b[2] * a[1], b[2] * a[0] - b[0] * a[2],
                                               b[0] * a[1] - b[1] * a[0]};
...
TEST_CASE("Property-based test: plucker linearity in first argument") {
    rc::check("plucker(k1*a + k2*b, c) == k1*plucker(a, c) + k2*plucker(b, c)", []() {
        ...
        auto k1a_k2b = std::array<int64_t, 3>{k1 * a[0] + k2 * b[0], k1 * a[1] + k2 * b[1],
                                              k1 * a[2] + k2 * b[2]};

        auto lhs = std::array<int64_t, 3>{k1a_k2b[1] * c[2] - k1a_k2b[2] * c[1],
                                          k1a_k2b[2] * c[0] - k1a_k2b[0] * c[2],
                                          k1a_k2b[0] * c[1] - k1a_k2b[1] * c[0]};

        auto cross_ac = std::array<int64_t, 3>{a[1] * c[2] - a[2] * c[1], a[2] * c[0] - a[0] * c[2],
                                               a[0] * c[1] - a[1] * c[0]};

        auto cross_bc = std::array<int64_t, 3>{b[1] * c[2] - b[2] * c[1], b[2] * c[0] - b[0] * c[2],
                                               b[0] * c[1] - b[1] * c[0]};

        auto rhs = std::array<int64_t, 3>{k1 * cross_ac[0] + k2 * cross_bc[0],
...
TEST_CASE("Property-based test: PgPoint scaled coordinates are equal") {
    rc::check("pg_point(coord) == pg_point(k*coord) for any non-zero k", []() {
        ...
        auto scaled = std::array<int64_t, 3>{k * coord[0], k * coord[1], k * coord[2]};
...
TEST_CASE("Property-based test: incident is symmetric for dot product") {
    rc::check("If point is incident to line, then line dot point is zero", []() {
        ...
        int64_t dot_value
            = pt_coord[0] * ln_coord[0] + pt_coord[1] * ln_coord[1] + pt_coord[2] * ln_coord[2];
...
TEST_CASE("Property-based test: parametrize identity") {
    rc::check("parametrize(1, p, 0, q) == p", []() {
        ...
        PgPoint pt_q(coord_q);
        PgPoint pt_result
            = PgPoint::parametrize(static_cast<int64_t>(1), pt_p, static_cast<int64_t>(0), pt_q);
...
TEST_CASE("Property-based test: parametrize linearity") {
    rc::check("parametrize(k1, p, k2, q) is linear", []() {
        ...
        auto expected = std::array<int64_t, 3>{k1 * coord_p[0] + k2 * coord_q[0],
