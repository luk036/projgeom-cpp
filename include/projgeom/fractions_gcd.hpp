...
namespace fun {
    ...
    constexpr auto abs(const T& a) -> T { return a; }

    template <typename T>
    requires(!std::is_unsigned_v<T> && ordered_ring<T>) constexpr auto abs(const T& a) -> T {
        return (a < T(0)) ? -a : a;
    }
    ...
    template <Integral Z> struct Fraction
         ...
         * @param[in] den
         */
        constexpr Fraction(const Z& num, const Z& den) : _num{num}, _den{den} { this->normalize(); }
